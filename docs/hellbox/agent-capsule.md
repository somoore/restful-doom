...
- For native in-process snapshot demos, set `SNAPSHOT_SAVE_SLOT_BASE=N` before `snapshot-capture`; captured stages can then restore through gRPC `LoadSnapshot` using `snapshot.slot` / `save_slot:N` instead of an external restore command.
...
- Confirm snapshot rollout rows include `info.reset_context.source == "snapshot_restore"` and `info.reset_context.restored_state_verification.valid == true`; summaries should include `snapshot_restore_count`, `snapshot_verification_count`, `snapshot_verification_failures == 0`, `snapshot_stage_counts`, and restore timing fields.
