...
- Live PPO checkpointing is proven, but independent learning is not yet proven. The next proof point is a longer multi-update PPO run that improves reward/kill metrics over random initialization and then challenges the deterministic baseline through the promotion gate.
